...
#ifdef USE_BLAS
void dcopy_(const unsigned *n, const double *x const unsigned *incx, 
            double *y, const unsigned *incy);
#endif

/* Copy a vector */
void copy(unsigned int n, const real * x, unsigned int incx, 
...
#ifdef USE_BLAS
void dsyr_(const char *uplo, const unsigned int *n, const real *alpha,
           const real *x, const unsigned int *incx, real *a,
           const unsigned int *lda);
#endif

void syr(bool lower, unsigned int n, real alpha,
         const real *x, unsigned int incx,
         real *a, unsigned int lda)
{
#ifdef USE_BLAS
  dsyr_(lower ? "Lower" : "Upper", &n, &alpha, x, &incx, a, &lda);
#else
  int j;
  ...
  {
    for (j = 0; j < n; j++)
      axpy(n - j, alpha * x[j * incx], x, incx, a + j + j * lda, 1);
  }
  else
  {
    for (j = 0; j < n; j++)
